...
export default function AIVisualizationPage() {
  ...
  const generateVisualization = async () => {
    if (!uploadedImage || !selectedProjectType || !selectedStyle) {
      return;
    }
    ...
    try {
      // Simuler la progression pendant l'appel API
      ...
      // Préparer le formulaire avec l'image
      ...
      // Utiliser l'image générée (base64 ou URL)
      ...
      alert(`Erreur: ${error.message || 'Impossible de générer la visualisation. Vérifiez que votre clé API OpenAI est configurée.'}`);
      setStep('configure');
    } finally {
  ...
  return (
    <PageWrapper>
      <header className="bg-black/20 backdrop-blur-xl border-b border-white/10 px-4 sm:px-6 py-4 ml-0 sm:ml-20">
                    ...
                    <Button 
                      ...
                      onClick={() => {
                        if (generatedImage) {
                          ...
                          link.download = `visualisation-${selectedProjectType}-${selectedStyle}.png`;
                          link.click();
                        }
                      }}
